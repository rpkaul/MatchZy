...
namespace MatchZy
{
    public partial class MatchZy
    {
        ...
        {
            PrintToPlayerChat($"{chatPrefix} {message}");
        }
        ...
        private void SendUnreadyPlayersMessage()
        {
            ...
            if (unreadyPlayers.Count > 0)
            {
                ...
                // PrintToPlayerChat($"{chatPrefix} Unready players: {unreadyPlayerList}. Please type .ready to ready up! {minimumReadyRequiredMessage}");
            ...
            else
            {
                ...
                if (isMatchSetup)
                {
                    // PrintToPlayerChat($"{chatPrefix} Current ready players: {ChatColors.Green}{countOfReadyPlayers}{ChatColors.Default}");
                    PrintToAllChat(Localizer["matchzy.utility.readyplayers", countOfReadyPlayers]);
                }
                else
                {
                    // PrintToPlayerChat($"{chatPrefix} Minimum ready players required {ChatColors.Green}{minimumReadyRequired}{ChatColors.Default}, current ready players: {ChatColors.Green}{countOfReadyPlayers}{ChatColors.Default}");
        ...
        private void SendSideSelectionMessage()
        {
            ...
            // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{knifeWinnerName}{ChatColors.Default} Won the knife. Waiting for them to type {ChatColors.Green}.stay{ChatColors.Default} or {ChatColors.Green}.switch{ChatColors.Default}");
        }
        ...
        private void HandleMatchStart()
        {
            ...
            {
                PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}MatchZy{ChatColors.Default} Plugin by {ChatColors.Green}WD-{ChatColors.Default}");
            }
        ...
        public void HandleClanTags()
        {
            ...
            if (readyAvailable && !matchStarted)
            {
                foreach (var key in playerData.Keys)
                {
                    ...
                    PrintToPlayerChat($"PlayerName: {playerData[key].PlayerName} Clan: {playerData[key].Clan}");
                }
            }
        ...
        private void HandleMatchEnd()
        {
            ...
            {
                PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{matchzyTeam1.teamName}{ChatColors.Default} is winning the series {ChatColors.Green}{matchzyTeam1.seriesScore}-{matchzyTeam2.seriesScore}{ChatColors.Default}");

            }
            else if (matchzyTeam2.seriesScore > matchzyTeam1.seriesScore)
            {
                PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{matchzyTeam2.teamName}{ChatColors.Default} is winning the series {ChatColors.Green}{matchzyTeam2.seriesScore}-{matchzyTeam1.seriesScore}{ChatColors.Default}");

            }
            else
            {
                PrintToPlayerChat($"{chatPrefix} The series is tied at {ChatColors.Green}{matchzyTeam1.seriesScore}-{matchzyTeam2.seriesScore}{ChatColors.Default}");
            }
        ...
        private void HandlePostRoundEndEvent(EventRoundEnd @event)
        {
            try
            {
                if (isMatchLive)
                {
                    ...
                    coachKillTimer = null;
                    (int t1score, int t2score) = GetTeamsScore();
                    PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{matchzyTeam1.teamName} [{t1score} - {t2score}] {matchzyTeam2.teamName}");
                    ...
                    {
                        PrintToPlayerChat($"{chatPrefix} The round restore request by {ChatColors.Green}{reverseTeamSides["CT"].teamName}{ChatColors.Default} was cancelled as the round ended");
                    }
                    else if (!stopData["ct"] && stopData["t"])
                    {
                        PrintToPlayerChat($"{chatPrefix} The round restore request by {ChatColors.Green}{reverseTeamSides["TERRORIST"].teamName}{ChatColors.Default} was cancelled as the round ended");
                    }
        ...
        private void PauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (isMatchLive && !isPaused)
            {
                ...
                PrintToAllChat(Localizer["matchzy.pause.pausedthematch", pauseTeamName]);
                // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{pauseTeamName}{ChatColors.Default} has paused the match. Type .unpause to unpause the match");
        ...
        private void ForcePauseMatch(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            unpauseData["pauseTeam"] = "Admin";
            PrintToAllChat(Localizer["matchzy.pause.adminpausedthematch"]);
            // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}Admin{ChatColors.Default} has paused the match.");
            if (player == null)
            {
        ...
        private void StartMatchMode()
        {
            ...
            ResetMatch();
            RemoveSpawnBeams();
            PrintToPlayerChat($"{chatPrefix} Match mode loaded!");
        }
