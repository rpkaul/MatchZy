...
namespace MatchZy
{
    ...
    public partial class MatchZy
    {
        ...
        public void StartPracticeMode()
        {
            ...
            PrintToPlayerChat($" {ChatColors.Green}Spawns: {ChatColors.Default}.spawn, .ctspawn, .tspawn, .bestspawn, .worstspawn");
            PrintToPlayerChat($" {ChatColors.Green}Bots: {ChatColors.Default}.bot, .nobots, .crouchbot, .boost, .crouchboost");
            PrintToPlayerChat($" {ChatColors.Green}Nades: {ChatColors.Default}.loadnade, .savenade, .importnade, .listnades");
            PrintToPlayerChat($" {ChatColors.Green}Nade Throw: {ChatColors.Default}.rethrow, .throwindex <index>, .lastindex, .delay <number>");
            PrintToPlayerChat($" {ChatColors.Green}Utility & Toggles: {ChatColors.Default}.clear, .fastforward, .last, .back, .solid, .impacts, .traj");
            PrintToPlayerChat($" {ChatColors.Green}Sides & Others: {ChatColors.Default}.ct, .t, .spec, .fas, .god, .dryrun, .break, .exitprac");
        }
        ...
        private void SpawnBot(CCSPlayerController botOwner, bool crouch)
        {
            try 
            {
                ...
                if (!unusedBotFound) {
                    // PrintToPlayerChat($"{chatPrefix} Cannot add bots, the team is full! Use .nobots to remove the current bots.");
        ...
        public void OnFFCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            PrintToPlayerChat($"{chatPrefix} Fastforwarding 20 seconds!");
