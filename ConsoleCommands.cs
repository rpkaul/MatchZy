    ...
    public partial class MatchZy
    {
        ...
        public void OnTeamStay(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (player.TeamNum == knifeWinner)
            {
                ...
                // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{knifeWinnerName}{ChatColors.Default} has decided to stay!");
                StartLive();
            }
        }
        ...
        public void OnTeamSwitch(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (player.TeamNum == knifeWinner)
            {
                ...
                // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{knifeWinnerName}{ChatColors.Default} has decided to switch!");
                StartLive();
            }
        }
        ...
        public void OnUnpauseCommand(CCSPlayerController? player, CommandInfo? command)
        {
            if (isMatchLive && isPaused)
            {
                ...
                else
                {
                    ...
                    // PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}{unpauseTeamName}{ChatColors.Default} wants to unpause the match. {ChatColors.Green}{remainingUnpauseTeam}{ChatColors.Default}, please write !unpause to confirm.");
                }
        ...
        public void OnEndMatchCommand(CCSPlayerController? player, CommandInfo? command)
        {
            if (IsPlayerAdmin(player, "css_endmatch", "@css/config"))
            {
                if (!isPractice)
                {
                    // PrintToPlayerChat($"{chatPrefix} An admin force-ended the match.");
        ...
        public void OnStartCommand(CCSPlayerController? player, CommandInfo? command)
        {
            if (IsPlayerAdmin(player, "css_start", "@css/config"))
            {
                ...
                {
                    //PrintToPlayerChat($"{chatPrefix} {ChatColors.Green}Admin{ChatColors.Default} has started the game!");
        ...
        public void OnAdminSay(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (player == null)
            {
                PrintToPlayerChat($"{adminChatPrefix} {command.ArgString}");
                return;
            }
            ...
            PrintToPlayerChat($"{adminChatPrefix} {message}");
        }
